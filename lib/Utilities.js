        ...
        try {
            ...
            */
            ...
            for (var i=0; i<count; i++) {
                if (fs.existsSync(this.TEMP + "video" + i + ".mp4") == true) {
                    command1 = command1.concat(" -i " + this.TEMP + "video" + i + ".mp4");
                    thestring = thestring.concat("file 'video" + i + ".mp4"+"'\n")
                }
            }
            command1 = command1.concat(" -filter_complex \"");
            ...
            for (i=0; i<realcount; i++) {
                command1 = command1.concat("[" + i + ":v:0][" + i + ":a?]");
                //command1 = command1.concat("[" + i + ":v][" + i + ":a]");
            }
            //realcount +=1;
            //command1 = command1.concat("concat=n=" + realcount + ":v=1:a=1[outv][outa]\" -map \"[outv]\" -map \"[outa]\"   -y " + out); 
            //System.out.println(command1);
            await fs.writeFileSync(this.TEMP+"theconcat.txt",thestring);
			command1=command1.concat("concat=n=" + realcount + ":v=1:a=1[outv][outa]\" -f concat -i "+this.TEMP+"theconcat.txt -map \"[outv]\" -map \"[outa]\" -y " + out); 
            return new Promise((res,rej) => {
                var _this = this
                setTimeout(function(){
    
                    cp.exec(_this.FFMPEG+command1,{},(error, stdout, stderr) => {
                        if(debug == true)
                            console.log(error)
                        if(error != null)
                            return rej(false)
